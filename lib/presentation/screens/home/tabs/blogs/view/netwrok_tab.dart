...
import 'package:intervyou_app/data/blogs_models/connections/Connections.dart';
...
class _NetwrokTabState extends State<NetwrokTab> {
  ...
  late List<UserInfoItem> suggestionsConnections = [];

  @override
  void didChangeDependencies() {
    viewModel = Provider.of<BlogsViewModel>(context, listen: false);
    Future.microtask(() {
        viewModel.fetchConnectionSuggestions();
      },
    );
  ...
  Widget build(BuildContext context) {
    return Consumer<BlogsViewModel>(builder: (context, value, child) {
      if (viewModel.suggestionConnectionsLoading) {
        return Center(
        ...
        suggestionsConnections = viewModel.suggestionConnections;
          ...
          children: [
            ...
            ListView.separated(
              itemBuilder: (context, index) => InkWell(
                  onTap: () {
                    UserArgumentsModel argumentsModel = UserArgumentsModel(suggestionsConnections[index].userId??'', viewModel);
                    Navigator.pushNamed(context, RoutesManger.userInfoProfile,
