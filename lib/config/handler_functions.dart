...
class HandlerFunctions {
  static void handleLogin(context, String email, String password) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (message.contains("Email not confirmed")) {
        ...
        Navigator.pushNamed(context, RoutesManger.emailConfirmationOtp, arguments: email);
  ...
  static void handleResendEmailConfirmationOtp(context, String email) async{
    ...
    if (response.statusCode == 200) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Email confirmation otp resent successfully')),
      );
    }
  ...
  static void handleForgotPassword(context, String email) async{
    ...
    if (response.statusCode == 200) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('If an account exists for this email and is confirmed, a password reset OTP has been sent.')),
      );
      Navigator.pushNamed(context, RoutesManger.forgotPasswordOtp,arguments: email);
    }
    if(response.statusCode == 400) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Invalid Credentials')),
  ...
  static void handleVerifyForgotPasswordOtp(context, String email, String otp) async {
    ...
    if (response.statusCode == 200) {
      ...
      Navigator.pushNamed(context, RoutesManger.resetPassword,arguments: email);
